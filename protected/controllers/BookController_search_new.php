...
class BookController extends Controller
{
    ...
    public function actionSearch()
    {
        ...
        $bookCriteria->addCondition('status=:status AND confirm=:confirm AND deleted=:deleted AND (SELECT COUNT(book_packages.id) FROM ym_book_packages book_packages WHERE book_packages.book_id=t.id) != 0');
            ...
            foreach($terms as $key => $term)
                if($term){
                    ...
                    if(!$personSql)
                        ...
                        $personSql .= " OR (";
                    $personSql .= "t.name_family regexp :term$key OR t.alias regexp :term$key)";
                    $personCriteria->params[":term$key"] = $term;
                    // category sql
                    if(!$categorySql)
                        $categorySql = "(";
                    else
                        $categorySql .= " OR (";
                    $categorySql .= "t.title regexp :term$key)";
                    $categoryCriteria->params[":term$key"] = $term;
                }
            $bookCriteria->addCondition($bookSql);
            $personCriteria->addCondition($bookSql);
            $categoryCriteria->addCondition($bookSql);
        }
        $pagination = new CPagination();
        $pagination->pageSize = 8;
        if(Yii::app()->request->isAjaxRequest)
        {
            $bookCriteria->limit=6;
            $personCriteria->limit=6;
            $categoryCriteria->limit=6;
            $pagination->pageSize = 6;
        }
        $bookDP = new CActiveDataProvider('Books' ,array(
            'criteria' => $bookCriteria,
            'pagination' => $pagination
        ));
        $personDP = new CActiveDataProvider('BookPersons' ,array(
            'criteria' => $personCriteria,
            'pagination' => $pagination
        ));
        $categoryDP = new CActiveDataProvider('Books' ,array(
