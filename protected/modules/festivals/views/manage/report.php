...
/* @var $model FestivalUsed */

Yii::app()->clientScript->registerScript("send-festival-used-form", '
    var report_type;

    $("body").on("submit", "#filter-form", function(e){
        e.preventDefault();
        var $filters = $(this).serialize();
        $.fn.yiiGridView.update("report-festival-used-list",{
            data: $filters
        });
    });
...
<h1>گزارش استفاده از طرح های افزایش اعتبار</h1>
...
<!--        --><?php //echo CHtml::dropDownList('FestivalUsed[method]', $model->method?$model->method:'', $model->methodLabels,array(
//            'class' => 'selectpicker',
//            'data-width' => '100%',
...
<!--    </div>-->
    <div class="row">
        ...
        <div class="form-group">
            <?php
            echo CHtml::radioButton('FestivalUsed[report_type]',(!$model->report_type || ($model->report_type && $model->report_type == '')?true:false), array(
        ...
        <div class="form-group">
            <?php
            echo CHtml::radioButton('FestivalUsed[report_type]',$model->report_type && $model->report_type == 'yearly'?true:false, array(
        ...
        <div class="form-group">
            <?php
            echo CHtml::radioButton('FestivalUsed[report_type]',$model->report_type && $model->report_type == 'monthly'?true:false, array(
        ...
        <div class="form-group">
            <?php
            echo CHtml::radioButton('FestivalUsed[report_type]',$model->report_type && $model->report_type == 'by-date'?true:false, array(
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    'columns' => array(
        ...
        array(
            ...
            'htmlOptions' => array('style' => 'width:50px'),
            'footer'=> Controller::parseNumbers(number_format($model->getTotalRangeAmount())).' تومان'
        ),
        array(
            'header' => 'میزان هدیه',
            'name' => 'gift_amount',
            ...
            'htmlOptions' => array('style' => 'width:50px'),
            'footer'=> Controller::parseNumbers(number_format($model->getTotalGiftAmount())).' تومان'
        )
    )
));
?>
