...
class PanelController extends Controller
{
    ...
    public function actionDiscount()
	{
        ...
        $criteria=new CDbCriteria();
        $criteria->with[] = 'app';
        $criteria->addCondition('app.publisher_id = :user_id');
        ...
        $booksDataProvider=new CActiveDataProvider('AppDiscounts', array(
            'criteria'=>$criteria,
        ));
    ...
    public function actionSales()
    {
        ...
        if(isset($_POST['show-chart'])) {
            ...
            $criteria->addCondition('date > :from_date');
            $criteria->addCondition('date < :to_date');
            $criteria->addInCondition('app_id',CHtml::listData($userBooks,'id','id'));
            ...
            for ($i = 0; $i < count($userBooks); $i++) {
                ...
                $count = 0;
                foreach ($report as $model) {
                    if ($model->app_id == $userBooks[$i]->id)
                        $count++;
                }
                $values[] = $count;
            }
