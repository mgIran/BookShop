...
$this->menu=array(
	array('label'=>'افزودن', 'url'=>Yii::app()->createUrl('/manageBooks/'.$this->controller.'/create')),
    array('label'=>'مدیریت', 'url'=>Yii::app()->createUrl('/manageBooks/'.$this->controller.'/admin')),
);
if(isset($_GET['step']))
    $step = (int)$_GET['step'];
?>
...
<h1>ویرایش برنامه <?php echo $model->id; ?></h1>
    ...
    <div id="general" class="tab-pane fade <?= ($step == 1?'in active':''); ?>">
        <?php $this->renderPartial('manageBooks.views.baseManage._form', array(
        ...
        <? if(!$model->getIsNewRecord()):?>
            <div id="packages" class="tab-pane fade <?= ($step == 2?'in active':''); ?>">
                <?php $this->renderPartial('manageBooks.views.baseManage._package', array('model'=>$model, 'dataProvider'=>$packageDataProvider)); ?>
            </div>
        <? endif;?>
        <? if(!$model->getIsNewRecord()):?>
            <div id="pics" class="tab-pane fade <?= ($step == 3?'in active':''); ?>">
                <?php $this->renderPartial('manageBooks.views.baseManage._imagesUpload', array('model'=>$model ,'images' => $images)); ?>
            </div>
        <? endif;?>
    </div>