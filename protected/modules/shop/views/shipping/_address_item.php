<?php
/* @var $data ShopAddresses */
/* @var $lastAddressID integer */
?>
<div class="address-item<?= Yii::app()->user->getState('delivery_address') == $data->id?" selected" : ($data->id == $lastAddressID ? ' selected' : '') ?>">
    <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12 radio-container">
        <div class="radio-control">
            <?php echo CHtml::radioButton("DeliveryAddress", Yii::app()->user->getState('delivery_address') == $data->id?true:($data->id == $lastAddressID ? true : false), array("id"=>"address-".$data->id, "value"=>$data->id));?>
